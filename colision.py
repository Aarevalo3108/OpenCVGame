...
def dibujar_forma(img, forma, x, y, size):
    if forma == 'rectangulo':
        ...
        pts = pts.reshape((-1, 1, 2))
        cv2.polylines(img, [pts], isClosed=True, color=(0, 0, 255), thickness=2)
    return pts if forma != 'rectangulo' else None

# Función para verificar colisión con un triángulo
def verificar_colision_triangulo(pelota_pos, pelota_radio, pts):
    for i in range(len(pts)):
        pt1 = pts[i][0]
        pt2 = pts[(i + 1) % len(pts)][0]
        if cv2.pointPolygonTest(pts, pelota_pos, True) >= -pelota_radio:
            return True
    return False

# Función para verificar colisión con un pentágono o rombo
def verificar_colision_poligono(pelota_pos, pelota_radio, pts):
    return cv2.pointPolygonTest(pts, pelota_pos, True) >= -pelota_radio
...
while True:
    ...
    for (forma, x, y, size) in obstaculos:
        pts = dibujar_forma(img, forma, x, y, size)
        h, w, c = img.shape
        cx, cy = int(lm.x * w), int(lm.y * h)
        ...
        if forma == 'rectangulo':
            ...
            if verificar_colision_triangulo((cx, cy), pelota_radio, pts):
                colision = True
        else:
            if verificar_colision_poligono((cx, cy), pelota_radio, pts):
                colision = True
